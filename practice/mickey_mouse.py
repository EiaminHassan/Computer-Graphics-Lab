...
# Mickey Mouse face coordinates (centered around 400, 300)
# Main face circle
FACE_CENTER_X = 400
FACE_CENTER_Y = 300
FACE_RADIUS = 150

# Left ear (top-left) - positioned so it touches the main circle
# Distance from center = FACE_RADIUS + EAR_RADIUS (so they touch)
LEFT_EAR_X = 300
LEFT_EAR_Y = 472
EAR_RADIUS = 50

# Right ear (top-right) - positioned so it touches the main circle
RIGHT_EAR_X = 500
RIGHT_EAR_Y = 472
...
# Right semicircle of D (using half circle)
MOUTH_SEMICIRCLE_CENTER_X = 390
MOUTH_SEMICIRCLE_CENTER_Y = 200
MOUTH_SEMICIRCLE_RADIUS = 25
...
def display():
    ...
    # Draw left eye inner circle (pupil)
    ...
    # Draw right eye inner circle (pupil)
    points = MidpointCircle(EYE_INNER_RADIUS, RIGHT_EYE_X, RIGHT_EYE_Y)
    for point in points:
        glVertex2f(point[0], point[1])
    
    # Draw vertical line from nose (center line)
    points = midPoint(400, 280, 400, 280 - 40)
    for point in points:
        glVertex2f(point[0], point[1])
    
    
    # Right semicircle of D (right half of circle only)
    points = MidpointCircle(MOUTH_SEMICIRCLE_RADIUS, MOUTH_SEMICIRCLE_CENTER_X, MOUTH_SEMICIRCLE_CENTER_Y)
    for point in points:
        # Only draw right half of the circle (x >= center_x)
        if point[0] >= MOUTH_SEMICIRCLE_CENTER_X:
            glVertex2f(point[0], point[1])
