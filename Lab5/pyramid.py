...
transX, transY, transZ = 0.0, 0.0, 0.0
...
def keyboard(key, x, y):
    ...
    elif key == b'w': transY += Ty
    elif key == b's': transY -= Ty
    elif key == b'd': transX += Tx
    elif key == b'a': transX -= Tx
    elif key == b'q': transZ += Tz
    elif key == b'e': transZ -= Tz
...
def init():
    ...
    glClearColor(0.95, 0.95, 0.92, 1.0)
...
def main():
    ...
    global stepX, stepY, stepZ
    global Tx, Ty, Tz
    global Sx, Sy, Sz

    print("=== 3D Pyramid Program ===")

    X1, Y1, Z1 = map(float, input("Enter X1 Y1 Z1 (base corner): ").split())
    X2, Y2, Z2 = map(float, input("Enter X2 Y2 Z2 (height & base size): ").split())

    stepX, stepY, stepZ = map(float, input("Enter rotation step values: ").split())
    Tx, Ty, Tz = map(float, input("Enter translation units: ").split())
    Sx, Sy, Sz = map(float, input("Enter scaling units: ").split())

    glutInit()
    glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGB | GLUT_DEPTH)
    glutInitWindowSize(700, 700)
    glutInitWindowPosition(100, 100)
    glutCreateWindow(b"3D Pyramid - Multi Color Faces")
